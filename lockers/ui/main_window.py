...
from PyQt5.QtWidgets import (
    ...
    QGraphicsView, QTableWidget, QTableWidgetItem, QWidget, QHBoxLayout,
...
from lockers.ui.view import LockerView
...
class LockerApp(QMainWindow):
    ...
    def _build_menubar_and_toolbar(self):
        ...
        a_save = QAction("Сохранить схему…", self)
        a_save.setShortcut("Ctrl+S")
        a_save.triggered.connect(self.save_layout)
        m_file.addAction(a_save)

        a_load = QAction("Загрузить схему…", self)
        a_load.setShortcut("Ctrl+O")
        a_load.triggered.connect(self.load_layout)
        m_file.addAction(a_load)

        a_bg = QAction("Загрузить фон…", self)
        a_bg.setShortcut("Ctrl+B")
        a_bg.triggered.connect(self.scene.load_background_via_dialog)
        m_file.addAction(a_bg)

        a_export_txt = QAction("Экспорт TXT…", self)
        a_export_txt.setShortcut("Ctrl+E")
        a_export_txt.triggered.connect(self.export_txt)
        m_file.addAction(a_export_txt)

        a_export_xlsx = QAction("Экспорт Excel…", self)
        a_export_xlsx.triggered.connect(self.export_excel)
        m_file.addAction(a_export_xlsx)
        ...
        a_new = QAction("Создать локер", self)
        a_new.setShortcut("Ctrl+N")
        a_new.triggered.connect(
            lambda: self.scene.add_locker(self.view.mapToScene(self.view.viewport().rect().center()))
        )
        m_add.addAction(a_new)

        a_grid = QAction("Создать сетку локеров…", self)
        a_grid.triggered.connect(
            lambda: self.scene.add_locker_grid(self.view.mapToScene(self.view.viewport().rect().center()))
        )
        m_add.addAction(a_grid)

        m_view = mb.addMenu("Вид")
        a_snap = QAction("Привязка к сетке", self, checkable=True)
        a_snap.setChecked(self.snap_to_grid)
        a_snap.triggered.connect(self._toggle_snap)
        m_view.addAction(a_snap)

        m_tools = mb.addMenu("Инструменты")
        a_capacity = QAction("Задать вместимость помещения…", self)
        a_capacity.triggered.connect(self.set_capacity_dialog)
        m_tools.addAction(a_capacity)

        m_about = mb.addMenu("О программе")
        a_auth = QAction("Авторы", self)
        a_auth.triggered.connect(self.show_authors)
        m_about.addAction(a_auth)
        a_about_what = QAction("О чем это…", self)
        a_about_what.triggered.connect(self.show_about_what)
        m_about.addAction(a_about_what)

        tb: QToolBar = self.addToolBar("Главная")
        tb.setMovable(False)
    ...
    def _do_autosave(self):
        try:
            ...
            self.statusBar().showMessage(f"Автосохранено: {os.path.basename(path)}")
        except Exception:
            pass
    ...
    def count_occupied(self):
        return self.state.occupied_count()

    def number_exists(self, num, exclude=None):
        return self.state.number_exists(int(num), exclude_obj=exclude)

    def next_free_number(self):
        return self.state.next_free_number()
    ...
    def _on_table_item_changed(self, item):
        ...
        if not isinstance(locker, LockerItem):
            return
        ...
        if col == 0:
            if text == "":
                ...
                try:
                    ...
                    self._reflect_locker_to_row(item.row(), locker)
                    return
    ...
    def show_authors(self):
        AuthorsDialog(self).exec_()

    def show_about_what(self):
        AboutWhatDialog(self).exec_()
    ...
    def on_edit_owner(self, it):
        ...
        if not self.can_assign_owner(it.owner, new_owner):
            return
        it.owner = new_owner
        it._update_color()
        self.sync_table_from_scene()
